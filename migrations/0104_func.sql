-- ============================================================================
-- Module: Core functions and triggers
-- Description: Create trigger helpers and business SQL functions.
-- ============================================================================
...
COMMENT ON FUNCTION get_user_basic_info(BIGINT) IS
    'Efficiently retrieves basic user information for authentication responses';
...
COMMENT ON FUNCTION get_user_permissions(BIGINT) IS
    'Efficiently retrieves all permissions for a specific user';
...
COMMENT ON FUNCTION get_login_credentials(VARCHAR) IS
    'Retrieves user credentials for login authentication, including lockout state';
...
COMMENT ON FUNCTION log_operation(BIGINT, VARCHAR, VARCHAR, TEXT, JSONB, INET, TEXT, VARCHAR, INTEGER) IS
    'Logs a single operation with automatic partition handling, including data field.';
...
BEGIN
    ...
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_created_at ON %I(created_at)', partition_name, partition_name);

    RAISE NOTICE 'Created partition: % for period % to %', partition_name, start_date, end_date;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION create_log_partition(DATE) IS
    'Creates a new partition for operation_logs table for the specified month';
