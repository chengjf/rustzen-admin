...
import { Form, Space, Tree, Typography, Tag, Checkbox, Card, Empty, Row, Col, Spin, Button } from "antd";
  ...
  const columns: ProColumns<Role.Item>[] = useMemo(() => [
    ...
    { 
      ...
      render: (_, entity, __, action) => (
        <Space>
          <RoleModalForm mode="edit" initialValues={entity} onSuccess={() => action?.reload()}>
            <a>编辑</a>
          </RoleModalForm>
          <AuthPopconfirm 
            title="确认删除吗？" 
...
const PermissionManager = ({ value = [], onChange, menuTree, loading }: any) => {
    ...
    const buildTree = (nodes: any[]): any[] => {
      return nodes.map(node => {
        ...
        return {
          ...
          children: node.children ? buildTree(node.children).filter(Boolean) : undefined
        };
      }).filter(Boolean);
    };
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center' }}><Spin tip="加载菜单中..." /></div>;

  return (
    <div style={{ display: 'flex', border: '1px solid #d9d9d9', borderRadius: 8, height: 450, overflow: 'hidden' }}>
      {/* 左侧：菜单树 */}
      <div style={{ width: 300, borderRight: '1px solid #d9d9d9', padding: 12, overflow: 'auto', background: '#fafafa' }}>
        <Tree
          checkable
          ...
          onCheck={(checkedInfo: any) => {
            ...
            const buttonIds = value.filter((id: number) => flatMap.get(id)?.menuType === 3);
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#fff' }}>
        <div style={{ padding: '12px 16px', background: '#f5f5f5', borderBottom: '1px solid #d9d9d9' }}>
          <Text strong>功能配置：{selectedKey ? flatMap.get(selectedKey)?.label : '请选择左侧菜单'}</Text>
        </div>
        <div style={{ flex: 1, padding: 16, overflow: 'auto' }}>
                ...
                <Col span={12} key={btn.value}>
                  <Card 
                    ...
                    onClick={() => {
                      ...
                      let next;
                      if (isChecked) {
                        next = value.filter((id: number) => id !== btn.value);
                      } else {
                        // 选中按钮，必须确保其所属菜单也被选中
                        next = Array.from(new Set([...value, btn.value, btn.parentId]));
                      }
                      onChange?.(next);
                    }}
                    style={{ 
                        cursor: 'pointer',
                        borderColor: value.includes(btn.value) ? '#1677ff' : '#f0f0f0', 
                        background: value.includes(btn.value) ? '#e6f4ff' : '#fff' 
                    }}
                  >
                    <Checkbox checked={value.includes(btn.value)} style={{ pointerEvents: 'none' }}>
                        {btn.label}
                    </Checkbox>
                  </Card>
                </Col>
          ...
          ) : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={selectedKey ? "该菜单下无功能按钮" : "请在左侧选择菜单"} />}
        </div>
      </div>
    </div>
  );
...
const RoleModalForm = ({ children, initialValues, mode, onSuccess }: any) => {
    ...
    if (open) {
      ...
      try {
        ...
        if (mode === 'edit' && initialValues) {
          ...
          const rawIds = initialValues.menus?.map((m: any) => m.value) || initialValues.menuIds || [];
  ...
  return (
    ...
    >
      <Row gutter={24}>
        <Col span={8}><ProFormText name="name" label="角色名称" rules={[{ required: true }]} /></Col>
        <Col span={8}><ProFormText name="code" label="角色编码" rules={[{ required: true }]} /></Col>
        <Col span={8}><ProFormSelect name="status" label="状态" options={ENABLE_OPTIONS} /></Col>
      </Row>
