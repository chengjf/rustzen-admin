...
import { Form, Space, Tree, Typography, Checkbox, Card, Empty, Row, Col, Spin, Button, message } from "antd";
        ...
        render: (_, entity) => (
          <Space>
            <AuthWrap code="system:role:update">
              <RoleModalForm mode="edit" initialValues={entity} onSuccess={handleReload}>
                <a>编辑</a>
              </RoleModalForm>
            </AuthWrap>
            <AuthPopconfirm
      ...
      <div style={{ width: 300, borderRight: "1px solid #d9d9d9", padding: 12, overflow: "auto", background: "#fafafa" }}>
        <Tree
          ...
          onCheck={(info) => {
            ...
            const currentButtonIds = value.filter(id => flatMap.get(id)?.menuType === 3);
...
const RoleModalForm = ({ children, initialValues, mode, onSuccess }: RoleModalFormProps) => {
  ...
  return (
    ...
    >
      <Row gutter={24}>
        <Col span={6}><ProFormText name="name" label="角色名称" rules={[{ required: true }]} /></Col>
        <Col span={6}><ProFormText name="code" label="角色编码" rules={[{ required: true }]} /></Col>
        <Col span={6}><ProFormDigit name="sortOrder" label="排序" initialValue={0} /></Col>
        <Col span={6}><ProFormSelect name="status" label="状态" options={ENABLE_OPTIONS} initialValue={1} rules={[{ required: true }]} /></Col>
      </Row>
